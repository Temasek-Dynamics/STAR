...
if __name__ == '__main__':
	...
	xmax, ymax = max(pixel_x) + 120, max(pixel_y) + 20
	...
	def sysOff():
		...
		for name, crazyflie in nodes.items():
			uri = crazyflie["uri"]
			subprocess.call(["ros2 run crazyflie reboot --uri " + uri + " --mode sysoff"], shell=True, timeout=2)

	def sysOn():
		nodes = selected_cfs()
		for name, crazyflie in nodes.items():
			uri = crazyflie["uri"]
			subprocess.call(["ros2 run crazyflie reboot --uri " + uri + " --mode syson"], shell=True, timeout=2)

	def allOff():
		nodes = selected_cfs()
		for name, crazyflie in nodes.items():
			uri = crazyflie["uri"]
			subprocess.call(["ros2 run crazyflie reboot --uri " + uri + " --mode alloff"], shell=True)
	...
	def checkBattery():
		...
		for name, crazyflie in nodes.items():
			...
			try:
				if not bigQuad:
					voltage = subprocess.check_output(["ros2 run crazyflie battery --uri " + uri], shell=True, timeout=2)
				else:
					voltage = subprocess.check_output(["ros2 run crazyflie battery --uri " + uri + " --external 1"], shell=True, timeout=2)
			except:
	...
	mkbutton(scriptButtons, "sysOn", sysOn)
	mkbutton(scriptButtons, "allOff", allOff)
