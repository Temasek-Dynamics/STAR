...
void cs2::cs2_application::tag_timer_callback()
{
        ...
        it != agents_tag_queue.end(); it++)
    {
            ...
            fact_it->second.observations.size() > observation_limit)
        {
            ...
            //Commented out as observation_threshold is a size, not a distance and is preventing poses from being
            //added.
            // double magnitude_1 = std::pow(10,-1);
            // double magnitude_2 = std::pow(10,-2);
            // double distance = trans.norm();
            // if (distance > observation_threshold)
            //     continue;

            // pose.sd_position =
            //     1/(observation_threshold - distance) * magnitude_1;
            // pose.sd_quaternion =
            //     1/(observation_threshold - distance) * magnitude_2;

            //Replace the above with sanity check
            static const float MAX_DISTANCE_ORIGIN = 50.0;
            const double distance = trans.norm();
            if (distance > MAX_DISTANCE_ORIGIN)
            {
                RCLCPP_INFO(this->get_logger(), "%s distance: %.3f MAX_DISTANCE_ORIGIN: %.3f", it->first.c_str(), distance, MAX_DISTANCE_ORIGIN);
                continue;
            }

            //Crazyflie firmware default values and logic:
            ...
            //too strong
            // pose.sd_position = 0.0001f;
            // pose.sd_quaternion = 0.0001f;
            // pose.sd_position = 0.01f;
            // pose.sd_quaternion = 0.01f;

            //too weak
            pose.sd_position = 1.0f;
            pose.sd_quaternion = 1.0f;

            RCLCPP_INFO(this->get_logger(), "%s publishing pose %.3f %.3f %.3f", it->first.c_str(), trans.x(), trans.y(), trans.z());

            pose_correction.poses.emplace_back(pose);
        }
        else
        {
            // RCLCPP_INFO(this->get_logger(), "else %lu %d", fact_it->second.observations.size(), observation_limit);
        }
