...
namespace cs2
{
    class cs2_application : public rclcpp::Node
    {
                ...
                : Node("cs2_application"), clock(RCL_ROS_TIME), tf2_bc(this)
            {
                ...
                for (const auto &name : cf_names) 
                {
                    ...
                    std::function<void(const Twist::SharedPtr)> vcallback = 
                        ...
                        this, std::placeholders::_1, --agents_states.end());
                    std::function<void(const Float32::SharedPtr)> bcallback = 
                        std::bind(&cs2_application::batt_callback,
                        this, std::placeholders::_1, --agents_states.end());
                    ...
                    batt_sub.insert({name, this->create_subscription<Float32>(
                        name + "/batt", 7, bcallback)});
        ...
        private:
            ...
            std::map<std::string, rclcpp::Subscription<Float32>::SharedPtr> batt_sub;
